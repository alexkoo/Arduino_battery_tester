/*
 на основе https://alexgyver.ru/capacity_tester/ но без дисплея
...
todo
 разобраться с неизвестными функциями
 Рассчет внутреннего сопротивления
 добавить функционал зарядки
 добавить функционал  внешней нагрузки
 управление через serial: Напряжения старт/стоп, внешняя нагрузка, внешнее сопротивление, количество циклов.
 Придумать норм вывод текущих значений
 Сохранение в eeprom последнего результата измерения и вывод его в сериал при подключении
*/


#define NUM_READS 100 // количество измерений при определении точного опорного напряжения

//settings
byte load_pin=0; //пин нагрузки (аналоговый!!!!)
byte res_pin=1; //пин нагрузки (аналоговый!!!!)
byte buzz_pin=2; //пин динамика
...
int serial_speed = 9600; // скорость порта
...
void loop() {
  ...
  if (V < Voff) { //выключение нагрузки при достижении минимального напряжения
    ...
    for (int i=0; i<5; i++) {  //выполнить 5 ра
